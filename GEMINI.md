...
## 🧪 테스트 및 교육용 주석 규칙
1. **테스트 전담**: Gemini는 Claude가 작성한 `test-spec.md`와 프로젝트의 [테스트 표준 가이드](document/testing-standards.md)에 따라 테스트 코드를 작성한다.
2. **교육용 주석 (Educational Commenting)**: 사용자의 학습을 위해 라이브러리 개념과 함수 역할을 상세히 주석으로 남긴다.
3. **결과 기록**: 테스트 결과를 `test-spec.md`에 수동으로 기록하고 체크박스를 업데이트한다.
